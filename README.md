...
Services>Data Management>Cloudant NoSQL Database
...
RELATED LINKS
    ...
    Cloudant developers section
    Using Cloudant from Python
    Using Cloudant from Node.js
    Using Cloudant from Java
    Using Cloudant from Ruby
    Cloudant tutorials on IBM developerWorks

IBM® Cloudant™ NoSQL DB for Bluemix™ is a NoSQL database as a service (DBaaS). It's built from the ground up to scale globally, run non-stop, and handle a wide variety of data types like JSON, full-text, and geospatial. Cloudant NoSQL DB is an operational data store optimized to handle concurrent reads and writes, and provide high availability and data durability.

Learn about Cloudant NoSQL DB with live demos and code examples in the Cloudant For Developers section, or get into the details with the API Documentation. If you're using Cloudant NoSQL DB for a mobile app, check out Data for iOS8.
You can access the service by any of the following methods:
...
Before we get started you need to install the CloudFoundry cf tool on your desktop. Instructions on how to do that can be found in the Bluemix Quick Start guide.
    ...
    Managing Cloudant NoSQL DB instance with CLI
...
Cloudant Node.js Client is the official Cloudant NoSQL DB library for Node.js, based on nano.
The Cloudant Node.js client requires user name, password, and account values from the Bluemix VCAP_SERVICES environment variable. These fields map onto the user name, password, and user name fields in VCAP_SERVICES.
Table 1. Cloudant Node.js fields mapped to VCAP_SERVICESCloudant Node.js 	VCAP_SERVICES in Bluemix
username 	username
password 	password
account 	username
Binding a Cloudant NoSQL DB to Liberty applications
...
The <service_type> is a lowercase string constant. For Cloudant NoSQL DB, the service_type is cloudantNoSQLDB. The <service_name> is the name of the service. For example, if you create a Cloudant NoSQL DB with the name "mycloudant", "mycloudant" is the service_name and the cloudant stanza ID is 'cloudantNoSQLDB-mycloudant'. If you provide configuration stanzas, your configuration IDs must be created with the necessary algorithms. Failure to comply with these conventions results in errors.
...
The following code is a sample of Cloudant NoSQL DB configuration that might be provided in the local server.xml file. This sample assumes that when the packaged server is pushed to the cloud, it is bound to a Cloudant NoSQL DB named "mycloudant". In this case, the jndiName attribute can be whatever value you prefer. The value of the jndiName attribute is independent of the service name.
